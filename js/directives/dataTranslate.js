 ...
 */
DuckieTV.directive('translate', [function () {
    ...
    updateText: function (elm, token) {
      ...
      for (var i = 0; i < values.length; i++) {
        if (values[i].substring(0, 4) == 'PLH_') {
          angular.element(elm).attr('placeholder', localizationStrings[values[i].substring(4)])
        } else if (values[i].substring(0, 4) == 'VAL_') {
          angular.element(elm).attr('value', localizationStrings[values[i].substring(4)])
        } else if (values[i].substring(0, 4) == 'TTL_') {
          angular.element(elm).attr('title', localizationStrings[values[i].substring(4)])
        } else if (values[i].substring(0, 4) == 'ALT_') {
          angular.element(elm).attr('alt', localizationStrings[values[i].substring(4)])
        } else if (values[i].substring(0, 4) == 'ABB_') {
          var sArray = values[i].substring(4).split('-')
          if (localizationStrings[sArray[0]] != undefined) {
            angular.element(elm).text(localizationStrings[sArray[0]].length < parseInt(sArray[1]) ? localizationStrings[sArray[0]] : localizationStrings[sArray[0]].substring(0, parseInt(sArray[1])) + '...')
          }
