    ...
    function(dialogs, $http, $q) {
        ...
        var notifications = {
            '1.1.4': ["<li>autoDownload: (new) use global include / exclude lists, (new) series custom search provider option, (new) option to ignore quality, include and excludes via series custom settings, (new) use global/custom size min/max, (new) use series custom search string, (fix) episodes with date scene names bug, (new) monitor auto-download activity via status sidepanel",
                ...
                "<li>searchEngines: (fix) remove Strike, KAT and torrentz as they are gone, (new) add 1337x, ExtraTorrent, IsoHunt, TorrentDownloads and Zooqle, (fix) replace showRSS.info with new.showRSS.info, (fix) bug in RarBG causing non-found results after 15 minutes, (fix) drop find random KAT mirror feature as rockaproxy is gone, (new) add 2160p search quality",
