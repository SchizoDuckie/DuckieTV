  ...
  function(dialogs, $http) {
    ...
    var notifications = {
      '1.1.6': [
        ...
        '<li>SearchEngines: (new) Add ETag (extratorrent.st), IsoHunt2 (isohunt.tv), KATws (kickass.ws), Knaben (knaben.org), TheRARBG (t-rb.org), Uindex(uindex.org), PiratesParadise (piratesparadise.org), FileMood (filemood.com).',
        '<li>SearchEngines: (del) Katcr is broken, TorrentZ2 is gone, ETTV is gone, Zooqle is gone, RarBG is gone, EzTV no results without cookie., TGx is gone, EXT hardcoded DL gone.',
        '<li>SearchEngines: (fix) limetorrrents DL links and new domain, 1337x DL links, TPB (*0.org), TPB mirror resolver (piratebayproxy.net), torrentdownloads new domain, fix details link for Jackett indexers, Isohunt new domain.',
        '<li>Standalone: (new) NWJS 0.87.0 with Chromium 124. Mac OSX ARM64 package now available.',
        '<li>Standalone: (fix) Prevent Chromium 112+ freezing tabs if idle. Switch chromium-args: --flag-switches-begin --disable-features=HighEfficiencyModeAvailable --flag-switches-end. Rework saving widow position.',
        '<li>TorrentClient: (new) Introducing tTorrent client, add support for qBt 5.2+ using APIkey.',
        '<li>TorrentClient: (fix) Replace deprecaded rTorrent calls, add support for qBitTorrent 4.2+, ignore aria2 metadata file reports. qBitTorrent 4.5+ fix API call to remove torrent. qBitTorrent 5.0+ fix API call to start/stop torrent, drop support for qBt prior to 4.1 . Upgrade Tixati API for 2.86+. Note that older versions are no longer supported.',
        '<li>TorrentDialogs: (fix) magnetLinks were being submitted twice due to NWJS bug. Trap SE errors so dialogues can report.',
        ...
        '<li>Trakt: (fix) Re-Register app to Trakt to fix latest 403. New required Oauth methods. Fix Trakt import not ticking posters on completion.',
