angular.module('DuckieTV.providers.tvragesync', ['DuckieTV.providers.tvrage'])
/**
    ...
    this.matchToTvRage = function(serie, episodes, scope) {
        ...
        var updateExisting = function(existing, update) {
            ...
            CRUD.FindOne('Episode', {
                ID_Episode: existing.ID_Episode
        ...
        self.TVRage.findEpisodes(serie.TVRage_ID).then(function(tvRageEpisodes) {
            ...
            for (var updateable, i = 0; i < episodes.length; i++) {
                ...
                updateable = tvRageEpisodes.filter(function(el) {
                    return matchSeason(el, existing) && matchEpisode(el, existing);
                });
            ...
            if (unMatched.length > 0) {
                console.log("Episodes found ! \n", unMatched.map(function(el) {
                    return ('S' + el.season + 'E' + el.episode)
                }).join(' \n '), '\n', unMatched, episodes);
