DuckieTV.controller("customSearchEngineCtrl", ["SettingsService", "TorrentSearchEngines", "$q", "$http", "$injector","$scope",
    function(SettingsService, TorrentSearchEngines, $q, $http, $injector, $scope) {
        ...
        this.fields = [{key: 'name', type: "input", templateOptions: { label: "Search Engine Name", type: "text" }},
            ...
            {key: 'testSearch', type: "input", templateOptions: { label: "Search query to use for testing", type: "text" }},
        ...
        this.test = function() {
           ...
           var testClient =  new GenericTorrentSearchEngine({
            ...
            testClient.search(self.model.testSearch).then(function(results) { self.status = results.length > 0 ? 'Working!' : 'No results for search query :( '; $scope.$applyAsync; });
        ...
        this.add = function() {
            ...
            this.status = 'Done! check the torrent ialog!';
